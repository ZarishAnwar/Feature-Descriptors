...
%% set path 
addpath('large_scale_svm');   %addpath('directory') adds the specified directory to the top (also called front) of the current MATLAB search path. 
addpath('sift');
addpath(genpath('sparse_coding')); % recursively add subdirectories of your directory in addition to the directory itself, run
...
% calculate sift features or retrieve the database directory
 if skip_cal_sift,
   ...
   % FileName = fullfile('E:\output database',sprintf('%d.jpg',database))
 
 end;

%% load sparse coding dictionary (one dictionary trained on Caltech101 is provided: dict_Caltech101_1024.mat)
%Bpath = addpath(fullfile('dictionary/dict_' dataSet '_' num2str(nBases) '.mat'));
...
for iter1 = 1:numFea,  
    ...
    else % caution: Now in sc_poolin g.m the dir is set as 'label_3'.
